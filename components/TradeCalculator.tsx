...
export default function TradeCalculator({ config }: { config: TradeCalculatorConfig }) {
  ...
  const [laborCostPerHour, setLaborCostPerHour] = useState(defaults.laborCostPerHour);
  const [vehicleFuelPerJob, setVehicleFuelPerJob] = useState(defaults.vehicleFuelPerJob);
  const [monthlyOverhead, setMonthlyOverhead] = useState(defaults.monthlyOverhead);

  const results = useMemo(() => {
    const revenue = jobsPerMonth * pricePerJob;
    const laborCost = jobsPerMonth * hoursPerJob * laborCostPerHour;
    const fuelCost = jobsPerMonth * vehicleFuelPerJob;
    const totalCost = laborCost + fuelCost + monthlyOverhead;
    const profit = revenue - totalCost;
    const marginPct = revenue > 0 ? (profit / revenue) * 100 : 0;
    const costPerJob = jobsPerMonth > 0 ? totalCost / jobsPerMonth : 0;

    return { revenue, laborCost, fuelCost, totalCost, profit, marginPct, breakEvenPrice: costPerJob };
  }, [jobsPerMonth, pricePerJob, hoursPerJob, laborCostPerHour, vehicleFuelPerJob, monthlyOverhead]);

  const marginColor =
    results.marginPct < 10 ? 'bg-red-500' : results.marginPct < 25 ? 'bg-amber-500' : 'bg-emerald-500';
  const marginLabel =
  ...
  return (
    <div className="w-full max-w-3xl mx-auto bg-surface border border-line rounded-2xl p-6 md:p-8">
        ...
        <div className="space-y-5">
          <div>
            <label className="block text-sm text-ink/70 mb-1">
              {jobUnitLabel.charAt(0).toUpperCase() + jobUnitLabel.slice(1)}s per month
            </label>
            <input
              type="number"
              min={0}
              value={jobsPerMonth}
              onChange={(e) => setJobsPerMonth(Number(e.target.value) || 0)}
          ...
          <div>
            ...
            <input
              ...
              min={0}
              value={pricePerJob}
              onChange={(e) => setPricePerJob(Number(e.target.value) || 0)}
              className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-ink placeholder-sand focus:outline-none focus:ring-2 focus:ring-accent-from"
            />
          </div>
        ...
        <div className="flex flex-col justify-between">
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-sm font-medium text-ink">{results.marginPct.toFixed(1)}%</span>
            </div>
          ...
          <div className="mt-6 rounded-xl bg-paper border border-line p-4">
            <p className="text-sm text-ink/70">Break-even price per {jobUnitLabel}</p>
            <p className="text-xl font-semibold text-ink mt-0.5">{fmt(results.breakEvenPrice)}</p>
      ...
      <div className="mt-8 pt-6 border-t border-line flex flex-col sm:flex-row items-center justify-between gap-3">
        <p className="text-sm text-ink/70">Want more customers finding pages like this one?</p>
